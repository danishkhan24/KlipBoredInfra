...
data "aws_eks_cluster" "eks_cluster" {
  name = aws_eks_cluster.eks_cluster.name  # Use your existing EKS cluster resource name
}
...
resource "aws_iam_policy" "aws_load_balancer_controller_policy" {
  ...
  policy = file("iam_policy.json")  # Assume you have downloaded the policy file
}
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${data.aws_eks_cluster.eks_cluster.identity[0].oidc[0].issuer}:sub" = "system:serviceaccount:kube-system:aws-load-balancer-controller"
          }
        }
      }
    ]
