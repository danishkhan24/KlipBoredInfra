    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(data.terraform_remote_state.eks.outputs.eks_cluster.identity[0].oidc[0].issuer, "https://", "")}:sub" = "system:serviceaccount:kube-system:aws-load-balancer-controller"
          }
        }
      }
    ]
